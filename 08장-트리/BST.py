...
def preorder(n):                                # 전위 순회(VLR)
    if n is not None:
        print(n.key, end=" ")                  # 먼저 루트노드 처리(화면 출력)
...
def search_max_bst(n):                          # 이진탐색트리의 가장 큰 값 탐색
    while n != None and n.right != None:
        n = n.right                             # 가장 오른쪽에 있는 값 확인
    return n

def search_min_bst(n):                          # 이진탐색트리 가장 작은 값 탐색
    while n != None and n.left != None:
        n = n.left                              # 가장 왼쪽에 있는 값 확인
    return n
...
def delete_bst(root, key):
    if root is None : return None               # 공백 트리
